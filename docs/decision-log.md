# 判断記録

## 目的

このファイルは、M5PaperS3 Lux / Env Slides プロジェクトで行った主要な設計判断と、その理由を残すための記録です。

## 判断履歴

### 1. 目的は高精度予報ではなく教材にする

判断:
- このプロジェクトは「雨が近いかもしれない」と考える体験を作る教材とする

理由:
- センサ値から自然現象を読む体験を重視するため
- 正解表示よりも観察と推理を優先するため

### 2. 画面構成は複数スライドに分ける

判断:
- 1画面に全情報を詰め込まず、役割ごとにスライドを分ける

理由:
- 中学生が段階的に読めるようにするため
- 現在値、変化、短期傾向、長期傾向を分離した方が理解しやすいため

### 3. 横置きではなく縦置きを採用する

判断:
- M5PaperS3 の表示は縦置きを基本にする

理由:
- 上から下へ読む教材UIと相性が良いため
- 右側の余白問題を避けやすいため

### 4. 通常ループは4枚の教材スライドに固定する

判断:
- 通常表示は `現在値 / 変化のサイン / 短期傾向 / 長期傾向` の4枚とする

理由:
- 役割分担が明快になるため
- 状態画面を主役にしないため

### 5. ネットワーク画面は補助画面にする

判断:
- `Status` 画面は通常ループに含めず、フッター操作でのみ開く補助画面にする

理由:
- 教材としての主役は観測と推理だから
- 通信状態は必要だが常時見せる優先度は低いため

### 6. 短期傾向と長期傾向を分ける

判断:
- `Slide 3` は短期 15 分
- `Slide 4` は長期 120 分

理由:
- 「今の変化」と「流れの継続」は別の意味を持つため
- 明るさは短期で読みやすく、気圧は長期で意味が出やすいため

### 7. 変化方向は文字と記号で補助する

判断:
- `ASCEND / DESCEND / STEADY` と矢印やゲージを組み合わせて見せる

理由:
- E-Ink では色による説明が使いにくいため
- 直感的な理解を助けるため

### 8. ゲージは連続量にだけ使う

判断:
- ゲージは変化量や方向の補助に使い、`Wi-Fi` や `MQTT` のような二値状態には使わない

理由:
- 不要な視覚ノイズを減らすため
- 状態表示は文字で十分だから

### 9. グラフは短期と長期で描画負荷を分ける

判断:
- 長期グラフは圧縮描画し、短期グラフは高密度のまま描く

理由:
- E-Ink の描画負荷を抑えるため
- 長期傾向は細かいノイズより全体方向が重要なため

### 10. 起動後もグラフ履歴を復元する

判断:
- SD の CSV ログから履歴を復元し、再起動後もグラフを継続表示できるようにする

理由:
- フラッシュ後にグラフが空になる体験を避けるため

### 11. 古い履歴は採用しない

判断:
- CSV 履歴はライブ受信時刻と比較し、古すぎる場合は破棄する

理由:
- 数時間前や前日の履歴を「今の傾向」と誤解させないため

### 12. フッター日時は受信した Unix time を使う

判断:
- M5PaperS3 自身の NTP 安定待ちではなく、受信データの `unix_time / ts` を JST に変換して表示する

理由:
- 送信側がすでに正しい時刻を持っているため
- 起動直後の長い `--:--` を避けられるため

### 13. バッテリー残量はヘッダー右上に置く

判断:
- ヘッダー右上に `BAT xx%` を表示する

理由:
- 補助情報として見やすく、スライド主役を邪魔しないため

### 14. 夜間は明るさを雨判定から外す

判断:
- 夜間候補かつ低照度継続時には、`Light` を雨の手がかり集計から外す

理由:
- 夜は暗いこと自体が自然であり、雨雲の影響と区別できないため

### 15. 夜間判定は時刻と照度の両方で行う

判断:
- `日没後/日の出前` だけではなく、`lux` の低照度継続も条件に含める

理由:
- 日没直後のまだ明るい時間や、一時的な暗さを誤判定しないため

### 16. 位置情報は `config.h` に出す

判断:
- 緯度・経度はスケッチ直書きではなく `config.h` で管理する

理由:
- 地域変更をしやすくするため
- 日の出・日没計算を設定可能にするため

### 17. 英語版と日本語版は同一リポジトリで管理する

判断:
- 別リポジトリに分けず、同一リポジトリで `config.h` により言語切替する

理由:
- バグ修正や機能追加を二重管理しないため
- 表示言語だけを分岐させれば十分だから

### 18. 日本語化は段階的に進める

判断:
- `日本語資産準備 -> 文言テーブル化 -> 日本語文言追加 -> レイアウト調整` の順で進める

理由:
- どこで壊れたか追いやすいため
- 英語版の安定状態を保ちやすいため

### 19. 日本語と数値は別フォントで扱う

判断:
- 日本語文言と、数値・単位・時刻を別系統で描画する

理由:
- 数字は既存フォントの方が見やすいため
- 日本語は日本語対応フォント経路が必要なため

### 20. 教材補助は「比較」と「問い」を重視する

判断:
- `今`
- `雨が近い並び`
- `何が先に変わった？`
- `もうすぐ雨が降るかな？`

のように、比較と問いをUIに入れる

理由:
- 正解を出しすぎず、考える流れを作るため
- 中学生が推理の形で学べるようにするため

### 21. 外部アイコン素材は SVG と組み込みデータを分けて管理する

判断:
- アイコン素材は `icons-src/` に置き、組み込み用データは `icons.h` に分けて管理する

理由:
- 差し替えや再生成をしやすくするため
- 著作権 / ライセンス記録と素材実体を追いやすくするため

### 22. アイコンは 16x16 ではなく 32x32 を採用する

判断:
- UI の主要アイコンは `32x32` を採用する

理由:
- E-Ink 画面上での視認性を上げるため
- 教材用途で、変化記号や状態記号を読み取りやすくするため

### 23. 昼間の雨の手がかり文言は意味が通る形へ明示する

判断:
- 日本語 UI の昼間ルール文言は `気圧↓ 湿度↑ 明るさ↓（昼の場合）` とする

理由:
- `（昼）` だけでは省略が強く、意味を補完しないと分かりにくいため
- 比較用の基準パターンであることを誤解されにくくするため

### 24. E-Ink の再描画は固定枠の再利用を優先する

判断:
- ヘッダ / フッタの固定枠と、スライド本文の再描画を分離する

理由:
- 同一スライド更新時の描画コストを下げるため
- 部分更新の前に、何を毎回描くべきかを整理する方が効果的なため

## 現在の方針

現在の方針は次のとおりです。

- 英語版は安定版として保持する
- 日本語版は同一リポジトリ内で仕上げる
- 主要な機能追加は一段落している
- 今後は読みやすさの微調整と、スライド本文側の再描画最適化を中心に進める
